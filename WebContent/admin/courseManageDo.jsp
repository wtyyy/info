	...
	<%
		out.println(user);
			if ((!"admin".equals(user.getPrivilege()))) {
				...
				return;
			} else if ("delete".equals((String) request
					.getParameter("oper"))) {
				...
				if (result > 0) {
					...
					response.sendRedirect("../message.jsp?message="
							+ URLEncoder.encode("查无此课，可能被别的管理员删了", "utf-8")
							+ "&redirect=/Test/admin/courseManage.jsp");
					return;
				}
				...
				if (CourseTime.fromDayAndBlock(course.getDay(),
					...
					response.sendRedirect("../message.jsp?message="
							+ URLEncoder.encode("无效的上课时间", "utf-8")
							+ "&redirect=/Test/admin/courseManage.jsp");
					return;
				}
				try {
					...
					if (course.getId() == -1) {
						st = con.prepareStatement("insert into courses(name, teacher,day,block, text, capacity, startTime, endTime, selectStartTime, selectEndTime)"
								+ "values(?,?,?,?,?,?,?,?,?,?)");
					} else {
						st = con.prepareStatement("update courses set name=?,teacher=?,day=?,block=?,text=?,capacity=?,startTime=?,endTime=?,selectStartTime=?,selectEndTime=? where id=?");
					...
					if (e.getErrorCode() == 1062) {
						response.sendRedirect("../message.jsp?message="
								+ URLEncoder.encode("课程名称已经存在", "utf-8")
								+ "&redirect=/Test/admin/courseManage.jsp");
						return;
					} else {
						response.sendRedirect("../message.jsp?message="
								+ URLEncoder.encode("数据格式有误", "utf-8")
								+ "&redirect=/Test/admin/courseManage.jsp");
						return;
					}
				}
				...
				int courseId = Integer.parseInt(request
						.getParameter("courseId"));
				String errorMessage = CourseSelect.select(studentId,
						courseId, true);
				...
				int courseId = Integer.parseInt(request
						.getParameter("courseId"));
				String errorMessage = CourseSelect.deselect(studentId,
						courseId);
				if (errorMessage == null) {
					response.sendRedirect(request.getParameter("redirect"));
				} else {
