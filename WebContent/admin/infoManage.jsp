				...
				<div class="full">
						...
						boolean slideModify = false;
							...
							if (operation != null) {
								if (operation.equals("add")) {
									...
									if (id == -1) {
										st = con.prepareStatement("insert into publicInfo(title, text, type) values(?,?,?)");
									} else {
										st = con.prepareStatement("update publicInfo set title=?,text=?,type=? where id="
												+ id);
									}
					...
					<form method="post" action="infoManage.jsp" name="addForm"
						...
						<ol>
							...
							<li><input type="radio" name="type" value="1"
								<%=isModify && modifyInfo.getType() == 1 ? "checked"
						: ""%> /><label>视频资源</label>
							</li>
							<li><input type="radio" name="type" value="2"
								<%=isModify && modifyInfo.getType() == 2 ? "checked"
						: ""%> /><label>音频资源</label>
							</li>
							<li><input type="radio" name="type" value="3"
								<%=isModify && modifyInfo.getType() == 3 ? "checked"
						: ""%> /><label>图片资源</label>
							</li>
					...
					<form method="get" action="infoManage.jsp">
						<table id="customers">
							...
							<%
								ResultSet rs = con.prepareStatement("select * from publicInfo")
											.executeQuery();
